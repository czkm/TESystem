...
<body>
	<form class="layui-form" action="${contextpath }/enrol/index.do" method="post">
		<table class="layui-table">
			...
			<tbody>
				<tr>
					<td colspan="8">
						...
						<div class="layui-inline" style="float: right" alt="添加" title="添加">
							<a href="${contextpath}/enrol/toedit.do" class="layui-btn">
				...
				<c:forEach items="${enrolList }" var="enrol">
					<tr>
						<td>${enrol.username}</td>
						<td>${enrol.userno}</td>
						<td>${enrol.major}</td>
						<td>${enrol.ranks}</td>
						<td>
						<select id="${enrol.id}" lay-filter='status'>
						<c:choose>
						<c:when test="${enrol.status==1}">
						...
						<td>
						<select id="${enrol.id }" data-id="${enrol.id }" lay-filter='department'>
								<c:forEach items="${departList }" var="depart" >
									<c:choose>
										<c:when test="${enrol.departid==depart.id }">
											<option value="${depart.id }" selected>${depart.departName}</option>
										</c:when>
										<c:otherwise>
											<option value="${depart.id }">${depart.departName}</option>
						...
						<td>
							<div class="layui-btn-group">
								...
								<a href="${contextpath}/enrol/toedit.do?id=${enrol.id}"
									class="layui-btn layui-btn-sm" alt="编辑" title="编辑"> <i
									class="layui-icon">&#xe642;</i>
								</a> <a href="${contextpath}/enrol/delete.do?id=${enrol.id}"
								 ...
								 <a href="javascript:;"
									...
									onclick="resetpassword(${enrol.id},'${enrol.userno}')"> <i class="layui-icon">&#xe6b1;</i>
                               </a>
							</div>
						</td>
					</tr>
		...
		layui.use('form', function() {
			...
			form.on('select(department)', function(data){
				  ...
				  $.ajax({
					  ...
					  url:"${contextpath }/enrol/changedepart.do",
					  dataType:"json",
					  beforeSend:function(){
