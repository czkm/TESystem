    ...
    -->
			<div class="layui-tab layui-tab-brief">
				<ul class="layui-tab-title" id="LAY_mine">
					<li data-type="mine-jie" lay-id="index" class="layui-this">教师评学
					</li>
				</ul>
				<div class="layui-tab-content" style="padding: 20px 0;">
					<form  class="layui-form layui-form-pane">
                    <input type="hidden" name="targetClasses" value="${assessClasses.targetClasses }">
                         <input type="hidden" name="userid" value="${assessClasses.userid }">
						 <input type="hidden" name="usertype" value="${assessClasses.usertype}">
						...
						<div class="layui-collapse">
							<c:forEach items="${criteriaClassesList }" var="criteria">
								<div class="layui-colla-item">
												<h3>${ criteria.criteriaName}(分值:${criteria.weight })</h3>
												<div>
													<c:forEach items="${ranks}" var="rank">
													
													<input type="radio" name="criteria${criteria.id}"
															value="${rank.value*criteria.weight/100}" title="${rank.key}"
															lay-filter="scorce ">
													</c:forEach>
												</div>
									</div>
		...
		layui.use([ 'element', 'form' ,'util'], function() {
			...
			form.on('submit(assesssubmit)', function(data) {
				...
				if (size > radioLength) {
				  ...
				  var targetClasses=data.field.targetClasses;
				  ...
				  var remarks=data.field.remarks;
				   ...
				   var json={
					    ...
					    $.ajax({
					    	method:"POST",
					    	url:"${contextpath}/user/assessclasses",
